...
class OffenseAgent(CaptureAgent):
    ...
    def evaluate(self, currentGameState, successor, action):
        """
        Evaluate the game state to determine the desirability.
        """
        features = self.getFeatures(currentGameState, successor)
        weights = self.getWeights()

        return sum(features[feature] * weights.get(feature, 0) for feature in features)

    def getFeatures(self, currentGameState, successor):
        """
        ...
        if len(capsules) != 0:
            ...
            features['minCapsuleDistance'] = min([self.getMazeDistance(myPos, c) for c in capsules])
        ...
        if len(scaredGhosts) > 0:
            scaredDistances = [self.getMazeDistance(myPos, g.getPosition()) for g in scaredGhosts]
            features['distanceToScaredGhost'] = min(scaredDistances)
        else:
...
class DefenseAgent(CaptureAgent):
    ...
    def getFeatures(self, gameState):
        ...
        myPos = myState.getPosition()
        width = gameState.getWalls().getWidth()
        ...
        if len(invaders) > 0:
            ...
            walls = gameState.getWalls().getWidth() * gameState.getWalls().getHeight()
            features['invaderDistance'] = float(minInvaderDist) / walls
        else:
        ...
        if self.red:
            ...
            patrolX = midWidth
        patrolPoints = [(patrolX, y) for y in range(height) if not gameState.hasWall(patrolX, y)]
        minPatrolDist = min([self.getMazeDistance(myPos, point) for point in patrolPoints])
        
        features['distanceToPatrol'] = float(minPatrolDist) / (width * height)
